...
df_A_B_C = pd.read_csv(io.StringIO(data))

# df_A_B_C = df.loc[(df['Released_Year'] >= 2010) & (df['Released_Year'] <= 2019)]

# Step 1: Convert boolean columns to 0 and 1
df_A_B_C['Free'] = df_A_B_C['Free'].replace({True: 1, False: 0})
df_A_B_C['Ad Supported'] = df_A_B_C['Ad Supported'].replace({True: 1, False: 0})
df_A_B_C['In App Purchases'] = df_A_B_C['In App Purchases'].replace({True: 1, False: 0})
df_A_B_C['Editors Choice'] = df_A_B_C['Editors Choice'].replace({True: 1, False: 0})

# Step 2: Convert categorical variables into dummy variables
df_A_B_C = pd.get_dummies(df_A_B_C, columns=['Category', 'Content Rating'])
...
df_A_B_C[numerical_columns] = scaler.fit_transform(df_A_B_C[numerical_columns])


X = df_A_B_C[df_A_B_C.columns.difference(['Daily_Avg_Installs'])]
y = df_A_B_C.Daily_Avg_Installs
